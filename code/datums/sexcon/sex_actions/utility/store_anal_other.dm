...
/datum/sex_action/store_ass_other/shows_on_menu(mob/living/user, mob/living/target)
	...
	if(!target.getorganslot(ORGAN_SLOT_GUTS))
		return FALSE
	var/obj/item/organ/filling_organ/guts/targetass = target.getorgan(/obj/item/organ/filling_organ/vagina)
	if(!targetass)
		return FALSE
...
/datum/sex_action/store_ass_other/on_start(mob/living/user, mob/living/target)
	...
	var/obj/item/organ/filling_organ/guts/targetass = target.getorgan(/obj/item/organ/filling_organ/vagina)
	if(istype(useditem, /obj/item/weapon/knife))
...
/datum/sex_action/store_ass_other/is_finished(mob/living/user, mob/living/target)
	...
	if(istype(useditem, /obj/item/weapon/knife) && prob(flubchance))
