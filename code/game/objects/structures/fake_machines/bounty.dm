/obj/structure/roguemachine/bounty
	name = "EXCIDIUM"
	desc = "Created by a fanatical sect of devout followers of Wanderer, this machine sets bounties."
...
/obj/structure/chair/freedomchair/crafted/attack_right(mob/living/carbon/human/A)
	...
	var/obj/item/clothing/face/old_mask = M.get_item_by_slot(ITEM_SLOT_MASK)
	if(old_mask)
		if(istype(old_mask, /obj/item/clothing/face/facemask/prisoner))
...
/obj/structure/chair/arrestchair/attack_right(mob/living/carbon/human/A)
	...
	for(var/l in buckled_mobs)
		M = l
	if(!ismob(M))
		say("Cannot begin skull structure analysis without a subject buckled to the Justice.")
	...
	if(correct_head)
		...
		if(old_mask)
			M.dropItemToGround(old_mask, TRUE)
		var/obj/item/clothing/face/facemask/prisoner/prisonmask = new(get_turf(M))
		prisonmask.bounty_amount = reward_amount
		M.equip_to_slot_or_del(prisonmask, ITEM_SLOT_MASK, TRUE)
		playsound(src.loc, 'sound/items/beartrap.ogg', 100, TRUE, -1)
	else
