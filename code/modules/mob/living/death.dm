			...
			for (var/mob/living/player in GLOB.player_list)
				...
				if (HAS_TRAIT(player, TRAIT_DEATHSIGHT))
					if (HAS_TRAIT(player, TRAIT_CABAL))
						...
						to_chat(player, span_warning("Veiled whispers herald the Nameless Death's gaze in my mind's eye as it turns towards [locale] for but a brief, singular moment."))
